...
Regarding binary curves, we do not use them here because they are not of widespread use, and this repository aims to support common use stuff. Michael Rosing's book *Implementing Elliptic Curve Cryptography* use them extensively to the point of showing how to create your own random elliptic curves and many other interesting stuff. Actually, the protocols for ```ECDH```, ```ECMQV``` and ```Elgamal``` here were adapted from the code from his book.
