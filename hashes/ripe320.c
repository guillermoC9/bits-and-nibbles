...
static void ripe320_function(ripe320_t *ctx)
{
...
void ripe320_init(ripe320_t *ctx)
{
	...
	ctx->estado[5] = 0x76543210;
	ctx->estado[6] = 0xfedcba98;
	ctx->estado[7] = 0x89abcdef;
	ctx->estado[8] = 0x01234567;
	ctx->estado[9] = 0x3c2d1e0f;
}
...
void ripe320_final(ripe320_t *ctx,void *h)
{
    int quedan;
    ...
    put_le32(hash + 32,ctx->estado[8]);
    put_le32(hash + 36,ctx->estado[9]);
