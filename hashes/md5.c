...
static void md5_function(md5_t *ctx)
{
...
void md5_final(md5_t *ctx,void *h)
{
    int quedan;
    ...
    if(quedan < 8)
    {
        memset(ctx->data + ctx->pos,0,quedan);
        md5_function(ctx);
        ctx->pos=0;
        quedan = 64;
    }

    memset(ctx->data + ctx->pos,0,quedan - 8);

    put_le64(ctx->data + 56,ctx->cont);
