...
static void md2_function(md2_t *ctx)
{
    unsigned char i, j, t;
    ...
    /* Block format:
        ...
        16 bytes of status;
        16 bytes of data;
        16 bytes status XOR data
    */

    memcpy(ctx->state+16,ctx->data,16);

    for(i=32,t=0;t<16;t++)
        ctx->state[i++]=ctx->state[t] ^ ctx->data[t];
    ...
    t=ctx->checksum[15];
    for(i=0;i<16;i++)
        t=ctx->checksum[i] ^= s_box[ctx->data[i] ^ t];
...
void md2_final(md2_t *ctx,void *hash)
{
    ...
    pad=(unsigned char)(16 - ctx->count);
    while(ctx->count < 16)
        ctx->data[ctx->count++]=pad;
