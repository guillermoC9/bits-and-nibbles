...
static void ripe128_function(ripe128_t *ctx)
{
    ...
    ctx->estado[1] = ctx->estado[2] + d1 + a2;
    ctx->estado[2] = ctx->estado[3] + a1 + b2;
    ctx->estado[3] = ctx->estado[0] + b1 + c2;
    ctx->estado[0] = d2 + c1;
}
...
void ripe128_final(ripe128_t *ctx,void *h)
{
    int quedan;
