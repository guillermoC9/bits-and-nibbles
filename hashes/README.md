...
Criptographic hashes are extremely complex to create, as they need to be not only intrinsic -the result is always the same when giving the same input-, but also non-commutative -```a + b``` is not the same as ```b + a```-. This ensures that they information they represent has not been changed and have been fed in the same order. One typical use would be a control version, where I hash the first version of a text. Then add the new modifications to the previous hash to obtain a new one. Since then, by hahsing the new document and comparing the hashes I can determine if the document not only is legit, but also if it is the result of the new document over the first one.

This is one of the uses of criptographic hashes that are not very well known. From a secret S, you hash it -lets say- a 100 times in a row so ```H100 = HASH(H99,HASH(...,HASH(S)))```, and then call H100 it my signature. I can then sign some info X by just hashing it with a previous version of the hash like ```XH = HASH(H90,X)``` and publish H90 with the info, so someone can verify my signature by hasing the info with ```H90```, and then hashing H90 ten times and checking that it becomes ```H100``. 
