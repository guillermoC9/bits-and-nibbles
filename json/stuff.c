...
wchar_t *wcsdupr(const wchar_t *a_duplicar,wchar_t *reusa,int *nlen)
{
    wchar_t *nuevo=NULL;
    ...
    if(a_duplicar)
    {
        ...
        len = (int) wcslen(a_duplicar); 
        
        tam = (len + 1) * sizeof(wchar_t);

        nuevo=(wchar_t *)realloc(reusa,tam);
        if(nuevo) 
        {
            memcpy(nuevo,a_duplicar,tam);
            if(nlen)
                *nlen = len;
        }
    }
