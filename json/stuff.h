...
#define flunlockfile    _unlock_file
...
/* --------------------------------------------------- *
   A reusable wcsdup(). 
   If 'reuse' is not NULL, it uses that pointer with
   realloc() to reuse it on the string.

   If 'nlen' is not NULL, it receives the number of
   chars of the new string.
 * --------------------------------------------------- */

wchar_t *wcsdupr(const wchar_t *str_to_dup,wchar_t *reuse,int *nlen);

/* -------------------------------------------------- *
   Return TRUE if 's' is NULL, the string where it 
   points has no characters or its just full of 
   spaces as per iswspace().
 * -------------------------------------------------- */
...
wchar_t *wcstrim(wchar_t *s);

/* -------------------------------------------------- *
   wcslcpy() is a better version of wcsncpy() as
...
size_t wcslcpy(wchar_t *s1, const wchar_t *s2, size_t max);
...
/* ----------------------------------------------------- *
   Convert a value 0-15 to hex char. 
   Cast it to wchar_t to use it with wide strings
 * ----------------------------------------------------- */

 char int_to_hex(int val);

/* ----------------------------------------------------- *
   Convert an hex wchar_t into a 0-15 value independant
   of case.
   Cast it to wchar_t from char to use it ANSI strings
 * ----------------------------------------------------- */

 int whex_to_int(wchar_t hex);
