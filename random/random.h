 ...
 * ----------------------------------------------------------- *
    ...
    Results for Mother:
        ...
        ---------   --------   ---------
           20,000      1           0
          200,000      5           0
        2,000,000    488           1 (0xDE72C23)
        3,000,000   1076           1 (0xDE72C23)
        5,000,000   2939           2 (0xDE72C23, 0x48281CB7)
        6,000,000   4195           5 (0xDE72C23, 0x48281CB7, 0xA1D98BAF, 0xA4D12C83, 0xB1BE31C1)
...
* ----------------------------------------------------------- *
    ...
    This is an attempt to write a true-random generator which
    produces random bytes for cryptography.

    I -Guillermo Amodeo Ojeda- wrote it at the start of 2018
    and called it GA-PRF because is uses a modified version
    of the P_HASH() pseudo-random function for TLS 1.2  using
    SHA-512 instead (GA stands for Generador Aleatorio, which
    in english means: 'Random Generator'. Sorry but could not 
    help myself to put some spanish here. ;-)

    I also tried my best to put into practice the theories
    explained on the section 4.2 (Guidelines for Designing a
    PRNG) of Schneier's paper 'Cryptanalytic Attacks on
    Pseudorandom Number Generators' where he said that it would
    be advisable to noise up a generator so the same entropy
    input takes different time and generates different output.

    This generator uses random_seed_no_linear() several times
    in each generation so it will disturb time in a way that cannot
    be used to build effective timing attacks on anythign that uses
    this generator.

    Given the nature of this generator, even if you save the state
    and load it again, the generated numbers will start to be
    different as soon as the remaining bits of the current hash
    are consumed. It would be very difficult that the new hash is
    the same in both contexts, although not imposible. The goal is
    to make the process of building a successful attack unfeasible. 
    
    Anyway the save/load functions are there to study the 
    algorithm because fixing any problem found with it would
    benefit all users of the generator.

    As with the previous, I tested it with test_random.c, and these
    are the average results (seed was also 1234):
        ...
        2,000,000     491          0
        3,000,000    1059          0
        5,000,000    2891          1 (0x8470B1B3)
        6,000,000    4198          2 (0x1794B3E3,0x8777E30E)
...
* ----------------------------------------------------------- *
    ...
    As with GA-PRF, saving the state and loading it again should
    yell different number sequences as soon as the remaining bits
    of the current buffer are consumed. Thus, it should be *very*
    complex that the new hash would be the same in two contexts,
    but is not imposible. Hopefully, it will be complex enough
    to make impractical to build an attack against it. But -again-
...
* ----------------------------------------------------------- *
    ...
    amount of generated numbers per second:
        ...
        Xorshift128      -->  41.0 seconds. (146,341.5 per second)
        Mother           -->  47.0 seconds. (127,659.6 per second)
        Mersenne Twister -->  51.0 seconds. (117,647.1 per second)
        GA-PRF           -->  Not yet tested on this system
        OSSYS            -->  Not yet tested on this system

    Results on a MacPro 2.8 Ghz Quad-Core Intel Xeon (Mid 2010).

        Xorshift128      -->   9.0 seconds. (666,666.6 per second)
        Mother           -->  11.0 seconds. (545,456.5 per second)
        Mersenne Twister -->  11.0 seconds. (545,456.5 per second)
        OSSYS            -->  14.0 seconds. (428,571.5 per second)
        GA-PRF           -->  Not yet tested on this system

    Results on a MacPro 3.5 Ghz 6-Core Intel Xeon E5 (Late 2013).

        Xorshift128      -->   3.2 seconds. (1,857,585.1 per second)
        Mother           -->   3.3 seconds. (1,796,945.2 per second)
        Mersenne Twister -->   3.3 seconds. (1,796,945.2 per second)
        OSSYS            -->   4.4 seconds. (1,363,636.4 per second)
        GA-PRF           -->   Not yet tested on this system

    Results on a MacBook Pro 3.1 Ghz Dual-Core Intel Core i7 (Early 2015).

        Xorshift128      -->   4.7 seconds. (1,276,595.7 per second)
        Mother           -->   4.7 seconds. (1,276,595.7 per second)
        Mersenne Twister -->   5.4 seconds. (1,111,111.1 per second)
        OSSYS            -->   6.0 seconds. (1,000,000,0 per second)
        GA-PRF           -->   Not yet tested on this system

    Results on a MacBook Pro 2.5 Ghz Quad-Core Intel Core i7 (Mid 2015).

        Xorshift128      -->   3.8 seconds. (1,578,947.3 per second)
        Mother           -->   4.0 seconds. (1,500,000.0 per second)
        Mersenne Twister -->   3.9 seconds. (1,538,461.5 per second)
        OSSYS            -->   4.7 seconds. (1,276,595,7 per second)
        GA-PRF           -->  28.9 seconds. (  207,612.4 per second)

    Results on a MacBook Pro M1 8 Core (2020).

        Xorshift128      -->   1.8 seconds. (3,333,333.3 per second)
        Mother           -->   1.9 seconds. (3,157,894.7 per second)
        Mersenne Twister -->   1.9 seconds. (3,157,894.7 per second)
        OSSYS            -->   2.5 seconds. (1,500,000,0 per second)
        GA-PRF           -->   Not yet tested on this system
    ...
    GA-PRF has not been tested with BigCrush (Any volunteer?) and
    neither attacked with crypto-analysis, but should be acceptable
    for cryptography as it uses the same algorithm than P_HASH()
    for TLS and tries to follow Bruce Schneier's advice.
...
enum
{
    ...
    RAND_GA_PRF,     /** GA_PRF. 'param' is a seed */
