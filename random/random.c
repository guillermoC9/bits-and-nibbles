...
#define SIZE_GA_PRF       SHA512_SIZE
#define SIZE_BUF_GA_PRF  (SIZE_GA_PRF * 2)

typedef struct
{
    unsigned char    sec[SIZE_GA_PRF];
    unsigned char    buf[SIZE_BUF_GA_PRF];
    size_t           left;
    unsigned char   *next;
} prf_rand_t;
...
/* our version of P_HASH() generates 2 x SIZE_GA_PRF bytes */

static void prf_phash_x2(prf_rand_t *rc)
{
    unsigned char a0[SIZE_GA_PRF],ai[SIZE_GA_PRF];
    sha3_t ctx;
    unsigned int s1;

    /* s1 its a non-linear unpredictable seed
      that also randomly disturbs time
    */

    s1=unpredictable_seed_non_linear();

    /* A(0) = p_hash(sec,s1) */

    sha512_init(&ctx);
    sha512_update(&ctx,rc->sec,SIZE_GA_PRF);
    sha512_update(&ctx,&s1,sizeof(s1));
    sha512_final(&ctx,a0);

    /* Generate SIZE_BUF_GA_PRF bytes ( 2 x SIZE_GA_PRF ) */

    /* A(1) = p_hash(sec,A(0),s1) */

    s1=unpredictable_seed_non_linear();

    sha512_init(&ctx);
    sha512_update(&ctx,rc->sec,SIZE_GA_PRF);
    sha512_update(&ctx,a0,SIZE_GA_PRF);
    sha512_update(&ctx,&s1,sizeof(s1));
    sha512_final(&ctx,ai);

    memcpy(rc->buf,ai,SIZE_GA_PRF);

    /* A(2) = p_hash(sec,A(1)) */

    sha512_init(&ctx);
    sha512_update(&ctx,rc->sec,SIZE_GA_PRF);
    sha512_update(&ctx,ai,SIZE_GA_PRF);
    sha512_final(&ctx,a0);

    /* A(3) = p_hash(sec,A(2),s1) */
    ...
    memcpy(rc->buf + SIZE_GA_PRF,ai,SIZE_GA_PRF);

    /* Prepare buffer for reading */

    rc->left = SIZE_BUF_GA_PRF;
    rc->next = rc->buf;

    /* Prepare next secret p_hash(sec,A(2),A(3)) */

    sha512_init(&ctx);
    sha512_update(&ctx,rc->sec,SIZE_GA_PRF);
    sha512_update(&ctx,a0,SIZE_GA_PRF);
    sha512_update(&ctx,ai,SIZE_GA_PRF);
    sha512_final(&ctx,rc->sec);
}

/* -------------------------- */

static void prf_rand_bytes(prf_rand_t *rc,void *data,size_t num)
{
    unsigned char *dest;
    size_t remain;
    int cnt;
...
static unsigned int prf_rand_function(prf_rand_t *rc)
{
    unsigned int ret;

    prf_rand_bytes(rc,&ret,sizeof(unsigned int));
...
static void prf_rand_init(prf_rand_t *rc,unsigned int seed)
{
    sha3_t ctx;
    unsigned int tmp;
    ...
    tmp=unpredictable_seed_non_linear();

    sha512_init(&ctx);
    sha512_update(&ctx,"GA_PRF_SHA_512",15);
    sha512_update(&ctx,&tmp,sizeof(unsigned int));
    sha512_update(&ctx,&seed,sizeof(unsigned int));
    sha512_final(&ctx,rc->sec);
...
static void ossys_bytes(ossys_rand_t *rc,void *data,size_t num)
{
    ...
    while(remain > 0)
    {
        if(rc->left == 0)
        {
            ossys_generate(rc);
            /*
               Although this would happen very rarely, a fail here
               could be caused by a temporary condition like the
               system booting/shutting down, the session logging
               off or a fork of the process.

               Remeber this code will only execute after a
               succesful call to ossys_function(rc) during the
               init phase, thus, to avoid a problem, we generate
               a block of  unpredictable random sequence using
               entropy while the condition goes away.
            */
            //if(rc->which==-1)
...
rand_t *rand_start(int cual,unsigned int param)
{
    ...
    switch(cual)
    {
        ...
        case RAND_GM:
            ...
            break;
        case RAND_GA_PRF:
            rc = rand_create(sizeof(prf_rand_t),(rand32_func_t)prf_rand_function,(rand8_func_t)prf_rand_bytes,NULL,NULL);
            if(rc)
            {
                rc->namec = "GA-PRF";
                rc->namew = L"GA-PRF";

                prf_rand_init((prf_rand_t *)rc->ctx,param);
            }
            break;
         case RAND_OS:
