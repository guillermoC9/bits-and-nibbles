...
{
    /* AMJ: Avoid a type-punning warning using a C99's method */
     union retval {
         const crc_table_t FAR  *table1;
         const unsigned long FAR *table2;
    } rv;
    rv.table1=(const crc_table_t FAR  *)crc_table;
#ifdef DYNAMIC_CRC_TABLE
    if (crc_table_empty)
        make_crc_table();
#endif /* DYNAMIC_CRC_TABLE */
