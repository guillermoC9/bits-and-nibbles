...
#define CHECK_FRAME( frame, n )  /* nothing */
  ...
  struct  TStream_Rec_
  {
    ...
    /* on-memory support  (written by Guillermo Amodeo) */

    const void *ptr;                    /*  Pointer to ttf in memory */
  };
 ...
 ******************************************************************/
                           ...
                           TT_Stream*  copy )
  {
    ...
    if(rec->ptr)
        return TT_Open_Mem_Stream(rec->ptr,rec->size,copy);
 ...
 ******************************************************************/
  ...
  /******* IGUAL que la anterior pero con el font en memoria ******/

LOCAL_FUNC  TT_Error  TT_Open_Mem_Stream(const void *ptr,Long tam,TT_Stream *stream )
{
    TT_Error     error;
    PStream_Rec  stream_rec;
 ...
 ******************************************************************/
  ...
  TT_Error  TT_Seek_File( STREAM_ARGS Long  position )
  {
    ...
    if(CUR_Stream->ptr)
    {
        if(position < 0 || position >= CUR_Stream->size)
            return TT_Err_Invalid_File_Offset;
        CUR_Stream->position = position;
    }
    else
    {
 ...
 ******************************************************************/
  ...
  TT_Error  TT_Skip_File( STREAM_ARGS Long  distance )
  {
    ...
    distance+=CUR_Stream->position;
    if(distance >= CUR_Stream->size)
        return TT_Err_Invalid_File_Offset;
    CUR_Stream->position = distance;
    return TT_Err_Ok;
  }
 ...
 ******************************************************************/
  ...
  {
    unsigned char *ptr = (unsigned char *)CUR_Stream->ptr;

    if(ptr)
    {
        Long position = CUR_Stream->position + count;

        if(position >= CUR_Stream->size)
            return TT_Err_Invalid_File_Read;

        memcpy(buffer, ptr + CUR_Stream->position,count);
